...
Изменения v0.5: агент сам проверяет погоду в Live и ведёт журнал (§6.5) · сценарий «сбой погоды» (§6.3).
...
- `scenario = "weather_outage"` (необязательно) — кнопка «Имитировать сбой погоды»: первый запрос погоды падает,
  агент сам повторяет, переходит на запасной путь и предупреждает. По умолчанию `null` — обычный запуск.
...
- **Агент работает сам.** Бэкенд раз в `LIVE_WATCH_MINUTES` минут (по умолчанию 30, `0` — выключено; первая
  проверка — через ~10 с после старта) запускает агента для `live`: нет выпуска — `trigger = "issue"`, есть —
  `"new_weather_run"`. Агент сам решает, пересчитывать ли. Каждый запуск Live, свой и по кнопке, пишется в
  `outputs/live/journal.jsonl`.
- `GET /api/live/journal?limit=20` → новые сверху:
  `[{"ts", "initiator": "agent | user", "trigger", "run_id", "outcome": "published | kept | refused | error", "version", "title"}]`
  (`title` — итог агента по-русски, например «Вышел прогон 06 UTC, сдвиг 1,1 м/с — выпустил v2»).
- `/health` дополнительно: `"live_watch_minutes"`, `"live_last_check_local"`.
...
2. **Live · сейчас** (`/api/live/status`): текущее время, последний и следующий прогон; «▶ Выпустить прогноз сейчас»,
   «Проверить обновления погоды»; тот же график и панель агента. **«Журнал агента»** (`/api/live/journal`) — что агент
   сделал сам, без людей (`initiator = agent`). Кнопка «Имитировать сбой погоды» (`scenario = weather_outage`) —
