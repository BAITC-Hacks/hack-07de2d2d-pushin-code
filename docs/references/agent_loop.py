...
def schema(fn):
    ...
    props = {
        k: {
            "type": "array"
            if v.annotation is list or getattr(v.annotation, "__origin__", None) is list
            else "string"
        }
...
async def agent(run_id: str, task: str):
    ...
    msgs = [
        {
            "role": "system",
            "content": "Ты агент-допускающий. Используй инструменты. Последний шаг — всегда действие.",
        },
        {"role": "user", "content": task},
    ]
    for step in range(8):
        r = client.chat.completions.create(
            model="gpt-5", messages=msgs, tools=[schema(t) for t in TOOLS]
        )
        m = r.choices[0].message
        msgs.append(m)
        if m.content:
            emit(run, "thought", m.content[:120], m.content)
        if not m.tool_calls:
            emit(run, "verdict", "Итог", m.content or "", artifact=run.get("artifact"))
            break
        for tc in m.tool_calls:
            ...
            out = fn(**args)
            kind = (
                "action" if (fn.__doc__ or "").startswith("ACTION") else "tool_result"
            )
            if kind == "action":
                run["artifact"] = out
            emit(
...
async def get_run(run_id: str):
    r = RUNS[run_id]
    return {"done": r["done"], "artifact": r["artifact"], "events": r["events"]}
